...
int main()
{
    ...
    rep(t,tc){
        ...
        sort(all(vec2));
        vec[0]=min(vec[0],vec2[0]-vec[0]);
        rep(i,n-1){
            int ind=lower_bound(all(vec2),vec[i-1]+vec[i])-vec2.begin();
            if(ind>=m){
                continue;
            }
            if(vec[i]<vec[i-1]){
                vec[i]=vec2[ind]-vec[i];
            }
            else{
                vec[i]=min(vec[i],vec2[ind]-vec[i]);
            }
        }
        puts(check(vec)?"YES":"NO");
    }
}