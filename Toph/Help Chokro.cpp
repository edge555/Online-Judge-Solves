...
int main()
{
    ...
    csum[0]=0;
    rep(i,n)
        csum[i]=csum[i-1]+vec[i-1];
    ll ans=1e18,tot=csum[n];
    for(i=0,j=0;j<=mx/k+2;i+=k,j++)
    {
        ll ind=lower_bound(all(vec),i)-vec.begin();
        if(ind==n)
        {
            ll needsum=n*i;
            ll need=needsum-tot;
        ...
        else
        {
            ...
            need=needsum-have;
            ll rightele=n-ind;
            ll rightsum=tot-have;
            needsum=rightele*i;
            need+=rightsum-needsum;
            ans=min(ans,need);
        }
    }
    pf("%lld\n",ans);
