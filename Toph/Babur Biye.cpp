...
string a;
int dp[N][3][3];
int func(int ind,int change,int last)
{
    if(ind==a.size())
        return 1;
    if(dp[ind][change][last]!=0)
        return dp[ind][change][last];
    int ans=0,ans2=0,ans3=0;
    if(last==0)
    {
        if(lower(a[ind]))
            ans=1+func(ind+1,1,1);
        else
            ans=1+func(ind+1,1,2);
        ans2=func(ind+1,change,last);
    }
    else if(last==1)
    {
        if(lower(a[ind]))
            ans=1+func(ind+1,change,last);
        else
        {
            if(change<3)
                ans=1+func(ind+1,change+1,2);
        }
        ans2=func(ind+1,change,last);
    }
    else
    {
        ...
        {
            if(change<3)
                ans=1+func(ind+1,change+1,1);
        }
        ans2=func(ind+1,change,last);
    }
    return dp[ind][change][last]=max(ans,ans2);
}
int main()
{
    #ifndef ONLINE_JUDGE
